...
import { map, reduce, keys, union, get, set, includes, omit, pick, isFunction, isArray, isString, isPlainObject } from 'lodash';
...
export default function (data, transforms, exclude) {
  ...
  if (isString(transforms)) return select(data, [transforms]);

  if (isArray(transforms)) return select(data, transforms);

  if (isString(exclude)) exclude = [exclude];
